...
{
    class NexusCommand : ICommand
    {
        public string Command { get { return "nexus"; } }
        ...
        public void Execute(Player player, string[] args)
        {
            ...
            if (player.Decision == 3) 
            {
                player.SendInfo("Alright, not dying and nexusing.");
                player.ForceConditionEffect(new ConditionEffect()
                {
                    DurationMS = 0,
    ...
    class ForgeListCommand : ICommand
       ...
       class YesCommand : ICommand
       {
           ...
           public void Execute(Player player, string[] args)
           {
               ...
               else if(player.Decision == 3)
               {
                   ...
                   player.Death(player.HoldKiller, true);
                   player.HoldKiller = "";
               }
               player.Decision = 0;
           }
       ...
       class NoCommand : ICommand
       {
           ...
           public void Execute(Player player, string[] args)
           {
               ...
               {
                   player.SendInfo("OK, not dying.");
                   player.HP = (int)(player.Client.Character.MaxHitPoints/2);
                    for(int i = 0; i < 27; i++)
                    {
                        if (i == 16) continue;
                        player.ForceConditionEffect(new ConditionEffect()
