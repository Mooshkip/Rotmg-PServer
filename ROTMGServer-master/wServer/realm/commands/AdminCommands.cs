...
namespace wServer.realm.commands
{
    ...
    class DebugCommand : ICommand
    {
        ...
        public void Execute(Player player, string[] args)
        {
            player.SendInfo("ID: " + player.AccountId);
            player.SendInfo("Name: " + player.Name);
            player.SendInfo("Untagged Name: " + player.Client.Account.Name);
        }
    }
    class TestingCommand : ICommand
    {
        public string Command { get { return "testing"; } }
        public int RequiredRank { get { return 4; } }

        public void Execute(Player player, string[] args)
        {
            player.SendInfo("tests: pet");
            player.GivePet(Convert.ToInt16(args[0]));
        }
    }
    ...
    class KillEvery : ICommand
    {
        public string Command { get { return "killevery"; } }
        public int RequiredRank { get { return 4; } }
    ...
    class FreezeqCommand : ICommand
    {
        public string Command { get { return "freezeq"; } }
        public int RequiredRank { get { return 4; } }
        ...
        public void Execute(Player player, string[] args)
        {
            ...
            {
                player.Quest.ForceConditionEffect(new ConditionEffect()
                {
                    DurationMS = -1,
    ...
    {
        public string Command { get { return "take"; } }
        public int RequiredRank { get { return 3; } }
        public Boolean isTag(string tag)
        {
            string[] tags = { "[P]", "[Helper]", "[Dev]", "[HDev]", "[420BlazeIt]", "[GM]", "[Admin]", "[SwagMaster]" };
        ...
        public void Execute(Player player, string[] args)
        {
            if (args.Length < 2)
            {
                player.SendError("Use syntax /take <player> <slot numbers spaced out>");
            }
            else
            {
                string id = args[0];
                ...
                player.SendInfo("Name: " + id);
                Player target;
                if ((target = (Player)player.Owner.GetUniqueNamedPlayer(id)) == null)
                {
                    player.SendInfo("Player not found!");
                    return;
                }
                player.SendInfo("Player: " + target.Name);
                Boolean[] slots = new Boolean[12];
                string[] tru = args.Skip(1).ToArray();
                for (int i = 0; i<12;i++)
                {
                    if(tru.Contains(i.ToString()))
                    {
                        slots[i]=true;
                    }
                    else
                ...
                player.Add(target.Remove(slots));
                player.SendInfo("Success!");

            }
        }
    }
    class UnfreezeqCommand : ICommand
    {
        public string Command { get { return "unfreezeq"; } }
        public int RequiredRank { get { return 4; } }
    ...
    {
        public string Command { get { return "heal"; } }
        public int RequiredRank { get { return 2; } }
    
        public void Execute(Player player, string[] args)
        {
            player.HP = player.Client.Character.MaxHitPoints;
            player.MP = player.Client.Character.MaxMagicPoints;
        }
    }

    class FameCommand : ICommand
    {
        public string Command { get { return "fame"; } }
        public int RequiredRank { get { return 4; } }
        ...
        public void Execute(Player player, string[] args)
        {
            if(args.Length != 2)
            {
                player.SendInfo("Usage: /fame <regular/current> <amount>");
            }
            else
            {
                ...
                {
                    player.SendError("Type must be regular or current.");
                }
            }
        }
    }
    class KillqCommand : ICommand
    {
        public string Command { get { return "killq"; } }
        public int RequiredRank { get { return 3; } }
        ...
        public void Execute(Player player, string[] args)
        {
            ...
            if (player.Quest == null)
            {
                ...
                //player.Move(player.X + 0.5f, player.Y + 0.5f);
                //player.SetNewbiePeriod();
                /*ConditionEffect eff = new ConditionEffect(){
                    DurationMS = 1000000,
                    Effect = ConditionEffectIndex.Stasis
                };*/
                player.Quest.ForceConditionEffect(new ConditionEffect()
                {
                    DurationMS = -1,
                    Effect = ConditionEffectIndex.Dead
                });
            /*player.Owner.BroadcastPacket(new GotoPacket()
            {
                ObjectId = player.Id,
    ...
    class Testingboicommand : ICommand
    {
        public string Command { get { return "testingboi"; } }
        public int RequiredRank { get { return 2; } }
        ...
        public void Execute(Player player, string[] args)
        {
            for(int i = 1; i<=26; i++)
            {
                ...
                player.ApplyConditionEffect(new ConditionEffect()
                {
                    Effect = (ConditionEffectIndex)i,
                    DurationMS = -1
                });
                player.SendInfo("Success lel");
            }
            
        }
    }

    class Effkillcommand : ICommand
    {
        public string Command { get { return "effkill"; } }
        public int RequiredRank { get { return 2; } }
