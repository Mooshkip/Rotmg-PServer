...
namespace wServer.realm.entities
{
    ...
    public partial class Player : Character, IContainer, IPlayer
    {
        ...
        public string HoldKiller { get; set; }
            ...
            : base((short)psr.Character.ObjectType, psr.Random)
        {
            ...
            switch(psr.Account.Rank) {
                ...
                case 6:
                    Name = "[420BlazeIt] " + psr.Account.Name; break;
                case 7:
                    Name = "[SwagMaster] " + psr.Account.Name; break;
            }
        ...
        public Item[] Remove(Boolean[] slots)
        {
            Item[] removed = new Item[slots.Length];
            for (int i = 0; i < slots.Length; i++)
            {
                if(slots[i] && Inventory[i] != null)
                {
                    removed[i] = Inventory[i];
                    Inventory[i] = null;
                    SendInfo("Removed item " + removed[i].ObjectId + ":" + removed[i].ObjectType + " from slot " + i);
                }
                else
                {
                    removed[i] = null;
        ...
        public Boolean AskIfDead()
        {
            Decision = 3;
            HP = 1;
            ForceConditionEffect(new ConditionEffect()
            {
                DurationMS = -1,
                Effect = ConditionEffectIndex.Paused
            });
            SendInfo("Do you want to die? /yes, /no, or /nexus.");
            return false;
        }

        public void Death(string killer,Boolean force=false)
        {
            if (psr.Account.Rank>=4 && !force)
            {
                HoldKiller = killer;
                if(!AskIfDead())
                {
